...
def __from_env():
    ...
    if env_sparse_conv_backend is not None and env_sparse_conv_backend in ['none', 'spconv', 'torchsparse', 'flex_gemm', 'torch_native']:
...
def set_conv_backend(backend: Literal['none', 'spconv', 'torchsparse', 'flex_gemm', 'torch_native']):
    global CONV
    CONV = backend
